...
export async function initConfig({ datasets, deckgl, webgl2, globe } = {}) {
  ...
  const config = {
    ...
    datetimeRange: WeatherLayers.offsetDatetimeRange(CURRENT_DATETIME, 0,340).join('/'),
    datetimeStep: 1,
    datetimes: [],
    datetime: NO_DATA,
    ...(deckgl ? {
    ...
    ...(webgl2 ? {
      particle: {
        ...
        numParticles: 5000,
        maxAge: 15,
        speedFactor: 10, // dataset-specific
        width: 1.0, // dataset-specific
        color: colorToCss(WeatherLayers.DEFAULT_LINE_COLOR),
        palette: false,
        opacity: 0.2,
        animate: true,
    ...
    tooltip: {
      ...
      followCursorOffset: 32,
      followCursorPlacement: WeatherLayers.Placement.TOP,
    },
  };
