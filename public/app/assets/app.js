...
const I18N = {
  en: {
    ...
    premium_overlay_info: "Unlock 14-day forecasts and 3D Globe with ClimaTok Premium. Try Premium FREE for 3 days. No charge today · Cancel anytime.",
  ...
  es: {
    ...
    premium_overlay_info: "Desbloquea previsiones de 14 días y visualizacion 3D con ClimaTok Premium. Prueba Premium GRATIS 3 días. No se cobra hoy · Cancela cuando quieras",
...
window.__CLIMATOK_NATIVE_APP = true;
window.__CLIMATOK_NATIVE_PREMIUM_LOCK = false;
...
//         premium: true,
//         premiumUntil: until,
//         customerId: localStorage.getItem('climatok_customer_id') || null
//       });
//     } else {
//       setPremiumFromApp({
//         premium: false,
...
(function bootstrapPremiumFromStorage(){
  try {
    const nativeApp = !!window.__CLIMATOK_NATIVE_APP;
    ...
    if (until && until > now) {
      setPremiumFromApp({
        premium: true,
        premiumUntil: until,
        sub: localStorage.getItem('climatok_customer_id') || null,
        source: "bootstrap"
      });
    } else {
      // En iOS nativo no degradamos a false aquí.
      if (!nativeApp) {
        setPremiumFromApp({ premium: false, premiumUntil: 0, source: "bootstrap" });
      }
    }
  } catch (_) {}
})();
...
//   try {
//     if (customerId) {
//       localStorage.setItem('climatok_customer_id', String(customerId));
//     }

//     if (premiumFlag && premiumUntil) {
//       localStorage.setItem('climatok_premium_until', String(premiumUntil));
//     } else {
//       localStorage.removeItem('climatok_premium_until');
//     }
//   } catch (_) {}
...
// }
// function setPremiumFromApp(value) {
//   console.log("[premium] setPremiumFromApp recibido:", value);
...
//   console.log("[premium] IS_PREMIUM_USER =", IS_PREMIUM_USER);
...
// }
function setPremiumFromApp(value) {
  console.log("[premium] setPremiumFromApp recibido:", JSON.stringify(value));
  ...
  const nativeLock = !!window.__CLIMATOK_NATIVE_PREMIUM_LOCK;
  const nativeApp = !!window.__CLIMATOK_NATIVE_APP;

  // En iOS nativo, si ya hemos confirmado premium desde Swift,
  // no permitimos que el flujo web lo degrade a false.
  if (nativeApp && nativeLock && !premiumFlag) {
    console.log("[premium] ignorado downgrade a false por native lock. source =", source);
    return;
  }
...
// EDITADO
  ...
  if (nativeApp && premiumFlag && (source === "native" || source === "ios")) {
    window.__CLIMATOK_NATIVE_PREMIUM_LOCK = true;
  }

  console.log("[premium] IS_PREMIUM_USER =", IS_PREMIUM_USER, "source =", source, "nativeLock =", window.__CLIMATOK_NATIVE_PREMIUM_LOCK);
...
(function notifyNativePremiumReady() {
  try {
    if (
      window.webkit &&
...
// === Legend OFF por defecto (al cargar) ===
(function initLegendDefaultOff(){
  // Marca el botón OFF como activo
  if (typeof setLegendActive === 'function' && legendOffBtn) {
    setLegendActive(legendOffBtn);
  } else {
    legendOffBtn?.classList.add('active');
    legendOnBtn?.classList.remove('active');
  }

  // Oculta la leyenda (WL wrapper + MapTiler ctrl)
  if (typeof hideLegend === 'function') hideLegend();
})();
...
// Autoajuste para evitar las recargas a 0,6,12,18 y 24 +30 min


// Ajuste automático de seguridad en los primeros minutos del run nuevo (0/6/12/18Z)
...
//     // Si estamos en la "ventana peligrosa" tras un run nuevo
//     // (primeros 30 min de 00, 06, 12, 18Z) subimos la seguridad a 12h
//     const isSynopticBoundary = (utcH % 6) === 0;   // 0, 6, 12, 18
//     const inFirst30Min       = (utcM < 30);

//     if (isSynopticBoundary && inFirst30Min) {
//       SAFETY_LAG_HOURS = 12;
//       console.log('[AWS mirror] Ventana de arranque del run -> SAFETY_LAG_HOURS = 12');
//     } else {
//       SAFETY_LAG_HOURS = 6;
//       console.log('[AWS mirror] Fuera de ventana de arranque -> SAFETY_LAG_HOURS = 6');
//     }
//   } catch (e) {
...
function getTimelineDatetimesForDataset(datasetId, datetimesAll) {
  ...
  // Si el dataset está en AWS, asumimos cadencia horaria (y queremos usarla)
  const isAWS = (typeof AWS_DATASETS !== 'undefined') && !!AWS_DATASETS[ds];
  if (isAWS) return datetimesAll;
  ...
  // Para el resto (GFS en WL, AIFS, etc.) mantenemos rejilla de 3h
...
// Datasets interceptados por AWS (puedes añadir más)
  ...
  const AWS_DATASETS = {
   ...
   //'ecmwf_ifs/pressure_mean_sea_level': { defaultUnscale: [940, 1080] },
   //'ecmwf_ifs/temperature_2m_above_ground': { defaultUnscale: [218, 322] },
  //  'ecmwf_aifs/wind_10m_above_ground': { defaultUnscale: [-67, 67] },
  //  'ecmwf_aifs/total_cloud_cover': { defaultUnscale: [0, 100] },
  //  'ecmwf_aifs/convective_available_potential_energy_surface': { defaultUnscale: [0, 7500] },
  //  'ecmwf_aifs/geopotential_height_500mb': { defaultUnscale: [4710, 6000] },
  //  'ecmwf_aifs/wind_gust_surface': { defaultUnscale: [0, 60] }, // o lo que toque
...
// Animacion no disponible
    ...
    //  const datasets = await client.loadCatalog();
      const datasets = "";
      const config = await initConfig({ datasets, deckgl: true, webgl2: useWebgl2 });
...
async function warmUpFrames(dataset, datetimes, aroundISO) {
  ...
  for (const dt of pick) {
    const { image, image2, imageWeight, imageType, imageUnscale, bounds } =
      await client.loadDatasetData(dataset, dt, { datetimeInterpolate: true });
...
async function precacheDatasets(layer) {
  ...
  let hoursToPreload = IS_LOW_HEADROOM
    ? Math.min(typeof rangeMax === 'number' ? rangeMax : 340, 24)
    : (typeof rangeMax === 'number' ? rangeMax : 340);

// Si estamos en cadencia horaria (AWS), limita a 120h como pediste (evita 300+ requests)
const _step = config?.datetimeStep;
if (_step === 'PT1H') {
  hoursToPreload = Math.min(hoursToPreload, 120);
}
...
async function precacheDatasetFrames(datasetId, frameCount = rangeMax, batchSize = 1) {
  ...
  const run = async () => {
    try {
      ...
      const dsKey = String(datasetId || '').toLowerCase();
      const stepLocal = (typeof AWS_DATASETS !== 'undefined' && !!AWS_DATASETS[dsKey]) ? 'PT1H' : config.datetimeStep;

      const { datetimes } = await client.loadDatasetSlice(
        datasetId,
        config.datetimeRange.split('/'),
        { datetimeStep: stepLocal, ...__wlOpts() }
      );
...
function showGlobeMobileOnlyModal() {
  ...
  // Evita duplicados
  if (document.getElementById('globe-modal')) return;

  const overlay = document.createElement('div');
  overlay.id = 'globe-modal';
  overlay.style = `
    position: fixed;
    inset: 0;
  ...
  box.innerHTML = `
    ...
    <button id="go-app-download" style="
    ...
    <button id="close-globe-modal" style="
  ...
  // cerrar
  document.getElementById('close-globe-modal').onclick = () => {
    overlay.remove();
  };
 ...
 map.once('idle', () => {
    // ⬇️ Crear MarkerLayout cuando el estilo ya está listo
    markerManager = new maptilermarkerlayout.MarkerLayout(map, {
      layers: ["City labels"],
      markerSize: [40, 70],
      offset: [0, -10],
      markerAnchor: "center",
      filter: (feature) => ["city","village","town"].includes(feature.properties.class),
    });
   ...
   // hideLegend();
  // ⬇️ Muestra en consola la URL real que usa WL para este dataset/instante
  
  // logSampleWLUrl('gfs/pressure_mean_sea_level');  // o cualquier otro dataset

    // logFinalWLAsset('gfs/pressure_mean_sea_level');
...
const precipitationColoRamp = new maptilerweather.ColorRamp({
  stops: [
    { value: 0,    color: [224, 252, 253,  40] },
    { value: 0.2,  color: [120, 245, 255, 140] }, // hace visible el arranque
  ...
  if (alertsEnabled) {
    ...
    if (capHits && capHits.length) return;
            ...
            //  showPointerCallout(e.lngLat);
              ...
              showPointerPanelTemporarily(500000);
...
map.once('idle', async () => {
  ...
  try {
    const datasetCustom = 'gfs/pressure_mean_sea_level';
  ...
  return [
    new WeatherLayers.RasterLayer({
     ...
     // beforeId: ' countries-borders',
       beforeId: 'countries-layer',
                 beforeId: 'Country labels',
                 beforeId:'Town labels',
                 beforeId: 'Minor road outline',
         //      beforeId: 'Water'
    })
  ];
};
...
timeSlider.addEventListener("input", () => {
  ...
  if (msUnix > (endMs )) {
          //  windLayer.setOpacity(0); // Oculta capa de viento
           ...
           // windLayer.setOpacity(1); // Muestra capa de viento
  }
  ///// 
...
function showPointerPanelTemporarily(ms = 500000) {
  const el = document.getElementById('pointer-callout');
  if (!el) return;
  ...
  __pointerHideTimer2 = setTimeout(() => {
    el.style.display = 'none';
    if (typeof pointerStickyLngLat !== 'undefined') {
      pointerStickyLngLat = null;
    }
...
const WIND_DENSITY = isIOS() ? 10 : 10; // iOS suele tener problemas de rendimiento con muchas partículas, así que reducimos la densidad
...
if (LOCAL_DATASET_META[dataset]) {
  ...
  const isAWSdataset = !!AWS_DATASETS[dataset];
  const stepLocal = isAWSdataset ? 'PT1H' : config.datetimeStep;

  const { datetimes } = await client.loadDatasetSlice(
    dataset,
    config.datetimeRange.split('/'),
    { datetimeStep: stepLocal }
  );

  // Mantén config.datetimeStep coherente para precache/keys del layer activo
  config.datetimeStep = stepLocal;

  const datetimes3h = to3hCadence(datetimes); // aún útil para logs/compat
...
if (
  ...
  datetimesTimeline[0] <= config.datetime &&
  config.datetime <= datetimesTimeline[datetimesTimeline.length - 1] &&
  !forceUpdateDatetime
) {
  ...
  let closest = datetimesTimeline[0];
  for (const dt of datetimesTimeline) {
...
const opts = isAWS
  ? { datetimeInterpolate: true }               // usamos image/image2/weight del wrapper
  : { datetimeInterpolate: !config.datetimeInterpolate };
...
const opacityToUse = (opacity > 0.9) ? 0.5 : opacity;
...
// nextHour
 ...
 function resolveWindDatasetForCurrentModel() {
  ...
  if (currentDataset === 'gfs/wind_tropopause_noaa') {
    return { id: 'gfs/wind_tropopause_noaa', speedFactor: 20 };
  }
  ...
  if (currentDataset === 'gfs/temperature_500mb') {
    ...
    return { id: 'gfs/wind_10m_above_ground', speedFactor: 10 };
  }
 ...
 if (currentDataset === 'gfs/geopotential_height_500mb') {
  ...
  // Por defecto, GFS
  return { id: 'gfs/wind_10m_above_ground', speedFactor: 9 };
}
...
const __IS_NATIVE_APP = !!window.__CLIMATOK_NATIVE_APP;
