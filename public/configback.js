...
export async function initConfig({ datasets, deckgl, webgl2, globe } = {}) {
  ...
  const config = {
    ...
    datetimeRange: WeatherLayers.offsetDatetimeRange(CURRENT_DATETIME, 0, 360).join('/'),
    datetimeStep: 1,
    datetimes: [],
    datetime: NO_DATA,
    ...(deckgl ? {
...
// datetimeInterpolate
    ...
    unitSystem: WeatherLayers.UnitSystem.METRIC_KILOMETERS,
    ...
    ...(webgl2 ? {
      particle: {
        ...
        numParticles: 2000,
        maxAge: 30,
        speedFactor: 20, // dataset-specific
...
// datetimeInterpolate
  ...
  gui.addBinding(config, 'datetimeRange', { options: getDatetimeRangeOptions([1, 2, 5, 10, 15]) }).on('change', updateDatetimes);
  ...
  gui.addBinding(config, 'imageSmoothing', { min: 5, max: 10, step: 1 }).on('change', update);
  ...
  raster.addBinding(config.raster, 'opacity', { min: 0.40, max: 1, step: 0.01 }).on('change', update);
